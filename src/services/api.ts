import { AuthResponse, Bank, PaginatedResponse, Transaction, TransferData, User } from "../types"
...
const generateTransactions = (page: number, limit: number = 10): Transaction[] => {
  const categories = [
...
export const api = {
  ...
  async transfer(_data: TransferData): Promise<{ success: boolean; transactionId: string }> {
    await delay(2000)
