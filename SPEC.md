...
- [x] Validação de CPF (mínimo 11 dígitos)
- [x] Validação de senha (mínimo 6 caracteres)
- [x] Loading state no botão durante "autenticação"
- [x] Feedback de erro visual (border vermelha + mensagem)
- [x] Persistência de sessão com `Expo SecureStore`
- [x] Redirecionamento automático para `/home` se já logado
...
- [x] Valor do saldo formatado em BRL (`R$ 1.234,56`)
- [x] Toggle de visibilidade do saldo (olho aberto/fechado)
- [x] Avatar com iniciais do usuário (gerado dinamicamente)
- [x] Pull-to-refresh para atualizar dados
- [x] Dados mockados (não precisa chamar API real)
...
- [x] FlatList com `onEndReached` e loading indicator
- [x] Filtro funcional (cliente-side para o mock)
- [x] Pull-to-refresh
- [x] Empty state se não houver transações no filtro
- [x] Componente `<TransactionItem />` reutilizável
- [x] Memoização com `React.memo` e `useCallback`
...
- [x] Validação de todos os campos com **Zod**
- [x] Máscara de valor em tempo real
- [x] Loading state em todas as ações de rede
- [x] Feedback visual de erro por campo
- [x] Limpeza do formulário após transferência bem-sucedida
